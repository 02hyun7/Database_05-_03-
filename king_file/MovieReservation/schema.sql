...
/* ============================
 * 2. 새로 추가되는 테이블 생성
 *    (screen_type, layout_template,
 *     coupon 확장, 복합 결제/환불)
 * ============================ */

-- 스크린 타입
CREATE TABLE `screen_type` (
  `screen_type_id` bigint PRIMARY KEY AUTO_INCREMENT,
...
-- 스크린 레이아웃 템플릿
CREATE TABLE `screen_layout_master` (
...
CREATE TABLE `coupon_type` (
  `coupon_type_id` bigint PRIMARY KEY AUTO_INCREMENT,
  `code` varchar(50) UNIQUE NOT NULL,
  `name` varchar(100) NOT NULL,
  `description` text
);
...
-- 쿠폰 혜택
CREATE TABLE `coupon_benefit` (
  `coupon_benefit_id` bigint PRIMARY KEY AUTO_INCREMENT,
  `coupon_id` bigint NOT NULL,
  `benefit_type` varchar(20) NOT NULL,   -- PERCENT, FIXED, FREE_ITEM
  `discount_value` decimal(10,2),
  `item_target_type` varchar(20),        -- MOVIE_TICKET, SNACK_ITEM ...
  `item_target_id` bigint,
  `quantity` int DEFAULT 1
);

-- 쿠폰 조건
CREATE TABLE `coupon_condition` (
  `coupon_condition_id` bigint PRIMARY KEY AUTO_INCREMENT,
  `coupon_id` bigint NOT NULL,
  `condition_type` varchar(30) NOT NULL, -- MOVIE, SCREEN_TYPE, MIN_AMOUNT, MENU_ITEM ...
  `ref_id` bigint,
...
CREATE TABLE `booking_charge` (
  ...
  `booking_id` bigint NOT NULL,
  `charge_type` varchar(30) NOT NULL,    -- TICKET, SNACK, FEE, TAX, ADJUSTMENT ...
  `related_id` bigint,
...
CREATE TABLE `refund_allocation` (
  `refund_allocation_id` bigint PRIMARY KEY AUTO_INCREMENT,
...
CREATE UNIQUE INDEX `review_like_review_member_uniq`
  ON `review_like` (`review_id`, `member_id`);

ALTER TABLE `booking_seat`
  COMMENT = 'PRIMARY KEY (booking_id, seat_id)';
...
-- screen_layout_master → screen_type
ALTER TABLE `screen_layout_master`
  ADD CONSTRAINT `fk_template_screen_type`
    FOREIGN KEY (`screen_type_id`) REFERENCES `screen_type` (`screen_type_id`);

-- screen → screen_type (optional)
ALTER TABLE `screen`
  ADD COLUMN `screen_type_id` bigint NULL,
  ADD CONSTRAINT `fk_screen_screen_type`
    FOREIGN KEY (`screen_type_id`) REFERENCES `screen_type` (`screen_type_id`);

-- screen_layout → screen_layout_master (optional)
ALTER TABLE `screen_layout`
  ADD COLUMN `template_id` bigint NULL,
  ADD CONSTRAINT `fk_layout_template`
    FOREIGN KEY (`template_id`) REFERENCES `screen_layout_master` (`template_id`);

-- coupon → coupon_type (optional)
ALTER TABLE `coupon`
  ADD COLUMN `coupon_type_id` bigint NULL,
  ADD CONSTRAINT `fk_coupon_type`
    FOREIGN KEY (`coupon_type_id`) REFERENCES `coupon_type` (`coupon_type_id`);

-- coupon_benefit → coupon
ALTER TABLE `coupon_benefit`
  ADD CONSTRAINT `fk_coupon_benefit_coupon`
    FOREIGN KEY (`coupon_id`) REFERENCES `coupon` (`coupon_id`);

-- coupon_condition → coupon
ALTER TABLE `coupon_condition`
  ADD CONSTRAINT `fk_coupon_condition_coupon`
    FOREIGN KEY (`coupon_id`) REFERENCES `coupon` (`coupon_id`);

-- booking_charge → booking
ALTER TABLE `booking_charge`
  ADD CONSTRAINT `fk_booking_charge_booking`
    FOREIGN KEY (`booking_id`) REFERENCES `booking` (`booking_id`);

-- payment_allocation → payment, booking_charge
ALTER TABLE `payment_allocation`
  ADD CONSTRAINT `fk_payment_alloc_payment`
    FOREIGN KEY (`payment_id`) REFERENCES `payment` (`payment_id`),
  ADD CONSTRAINT `fk_payment_alloc_charge`
    FOREIGN KEY (`charge_id`) REFERENCES `booking_charge` (`charge_id`);

-- refund_allocation → refund, payment_allocation, booking_charge
ALTER TABLE `refund_allocation`
  ADD CONSTRAINT `fk_ref_alloc_refund`
    FOREIGN KEY (`refund_id`) REFERENCES `refund` (`refund_id`),
  ADD CONSTRAINT `fk_ref_alloc_pay_alloc`
    FOREIGN KEY (`payment_allocation_id`) REFERENCES `payment_allocation` (`payment_allocation_id`),
  ADD CONSTRAINT `fk_ref_alloc_charge`
    FOREIGN KEY (`charge_id`) REFERENCES `booking_charge` (`charge_id`);
