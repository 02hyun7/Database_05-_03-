    ...
    public Booking createBookingForMember(Long memberId, Long showId, List<Long> seatIds) {
        ...
        try {
            ...
            for (Long seatId : seatIds) {
                ...
                // 2-3) 가격 계산 (price + seat_type 이용)
                BigDecimal price = priceDao.findFinalPrice(showId, seatId);
                if (price == null) {
                    System.out.println("[ERROR] 가격 정보를 찾을 수 없습니다. showId=" + showId + ", seatId=" + seatId);
                    return null;
                }
